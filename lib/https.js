...
export const createPersonalBet = async ({ personalBet, month, year, userId }) => {
  const url = `${apiBetsUrl}crear-apuesta-personal/?month=${
    month + 1
  }&year=${year}`;
  const res = await fetch(url, {
    headers: {
...
export const editPersonalBet = async ({
  personalBet,
  id,
  userId
}) => {
  const url = `${apiBetsUrl}crear-apuesta-personal/${id}/edit`;
  const res = await fetch(url, {
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
