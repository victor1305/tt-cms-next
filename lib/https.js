...
export const searchHorses = async (horseData, token) => {
  const url = `${apiRacesUrl}cargar-carreras-por-caballo`;
  const res = await fetch(url, {
    headers: {
...
export const editValue = async ({ value, field, token, id }) => {
...
export const removeValue = async ({ valueId, horseId, token }) => {
  const url = `${apiRacesUrl}borrar-valor/${horseId}/${valueId}`;
  const res = await fetch(url, {
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
